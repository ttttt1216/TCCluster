...
def representative_trajectory_generation(cluster_segment: dict, min_lines: int = 3, min_dist: float = 2.0):
    ...
    """
    representive_point = defaultdict(list) #value为list的字典
    for i in cluster_segment.keys():
        cluster_size = len(cluster_segment.get(i))  # 确定该类的size
        sort_point = []  # [Point, ...], size = cluster_size*2，因为同时存储的是起点和终点
        rep_point, zerox_point,zeroy_point = Point(0, 0, -1), Point(1, 0, -1), Point(0, 1, -1)
        ...
        rep_point = rep_point / float(cluster_size)  # 对所有点的x, y求平局值 cardinality of a set 集合中不重复的元素个数

        # 计算平均方向的sin和cos
        cos_theta = rep_point.dot(zerox_point) / rep_point.distance(Point(0, 0, -1))  # cos(theta)
        sin_theta = rep_point.dot(zeroy_point) / rep_point.distance(Point(0, 0, -1))  # sin(theta)
        # sin_theta = math.sqrt(1 - math.pow(cos_theta, 2))  # sin(theta) TODO:sin的正负如何判断？
        print("-----",math.sqrt(1 - math.pow(cos_theta, 2)),sin_theta)
        ...
        for j in range(cluster_size):
            ...
            # 坐标轴变换后进行原有的segment修改，将原有的结果重新进行了赋值
